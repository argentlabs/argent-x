...
import { Config, ConfigPlugin, transform } from "@svgr/core"
...
type IconsConfig = {
  ...
  additionalPlugins?: ConfigPlugin[]
...
const ICONS_CONFIG: IconsConfig[] = [
  {
    ...
    additionalPlugins: [
      "@svgr/plugin-svgo",
    ] /** good for flat artwork but clobbers complex logos */,
...
const getIconNodesAndNames = async (config: IconsConfig) => {
  ...
  Object.entries(components).forEach(([id, component]) => {
    ...
    if (
      componentSuffix &&
      !componentNameElements.some((element) =>
        element.toLowerCase().includes(componentSuffix),
      )
    ) {
...
const generateIconsWithConfig = async (config: IconsConfig) => {
  const { displayName, additionalSvgConfig, additionalPlugins, outputFolder } =
    config
  console.log(`Cleaning ${displayName} output folder ${outputFolder}`)
  cleanOutputFolder(outputFolder)
  const lines = ["/** This file is auto-generated by `pnpm gen:icons` */", ""]
