<script lang="ts">
  ...
  export let enableArgentMobileWallet: boolean = true
  ...
  let handleClickMobile = async () => {
    setLoadingItem("argent-login")
    const { getStarknetWindowObject } = await import("@argent/login")
    const starknet = await getStarknetWindowObject({ chainId: "SN_GOERLI" })
    await cb(starknet)
  }
    ...
    <ul class="flex flex-col gap-3">
      {#if enableArgentMobileWallet}
        <li
          ...
          on:keyup={handleClickMobile}>
          <span class="w-8 h-8" />
          <p class="font-semibold text-base">
            Argent mobile
          </p>
          {#if loadingItem === "argent-login"}
            <div role="status">
              <svg
                aria-hidden="true"
                class="w-8 h-8 text-neutral-300 animate-spin dark:text-neutral-600 fill-neutral-600 dark:fill-neutral-300"
